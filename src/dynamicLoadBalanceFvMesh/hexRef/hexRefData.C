...
Foam::hexRef8Data::hexRef8Data(const IOobject& io)
{
    ...
    {
        IOobject rio(io);
        rio.rename("refinementHistoryMultiDim");
        bool haveFile = returnReduce
        (
            rio.typeHeaderOk<refinementHistoryMultiDim>(),
            orOp<bool>()
        );
        if (haveFile)
        {
            Info<< "Reading hexRef data : " << rio.name() << endl;
            refHistoryPtr_.reset(new refinementHistoryMultiDim(rio));
        }
    }
}
...
)
{
    ...
    if (procDatas[0].refHistoryPtr_.valid())
    {
        ...
        UPtrList<const refinementHistoryMultiDim> procRefs(procDatas.size());
        forAll(procDatas, i)
        {
...
void Foam::hexRef8Data::sync(const IOobject& io)
{
    ...
    if (hasHistory && !refHistoryPtr_.valid())
    {
        ...
        rio.rename("refinementHistoryMultiDim");
        rio.readOpt() = IOobject::NO_READ;
        refHistoryPtr_.reset(new refinementHistoryMultiDim(rio, mesh.nCells(), true));
    }
}
