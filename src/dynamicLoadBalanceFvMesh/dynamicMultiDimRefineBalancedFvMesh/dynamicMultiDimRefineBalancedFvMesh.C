...
#include "dynamicMultiDimRefineBalancedFvMesh.H"
...
namespace Foam
{
    defineTypeNameAndDebug(dynamicMultiDimRefineBalancedFvMesh, 0);
    addToRunTimeSelectionTable(dynamicFvMesh, dynamicMultiDimRefineBalancedFvMesh, IOobject);
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //
Foam::label Foam::dynamicMultiDimRefineBalancedFvMesh::topParentID(label p)
{
...
Foam::dynamicMultiDimRefineBalancedFvMesh::dynamicMultiDimRefineBalancedFvMesh
(
    const IOobject& io
)
:
    dynamicMultiDimRefineFvMesh(io),
    rebalance_(false)
{}
...
bool Foam::dynamicMultiDimRefineBalancedFvMesh::update()
{

    //Part 1 - Call normal update from dynamicMultiDimRefineFvMesh
    bool hasChanged = dynamicMultiDimRefineFvMesh::update();
    ...
    // Part 2 - Load Balancing
    {
        ...
        Switch enableBalancing = refineDict.get<Switch>("enableBalancing");

        if ( Pstream::parRun() && hasChanged )
        {
            const scalar allowableImbalance =
                refineDict.get<scalar>("allowableImbalance");
